 ...
 */
export function formatNumber(value: string | number, decimalPlaces: number = 2, accuracy: number = 2) {
  // 将输入值转换为浮点数，以便后续处理
  ...
  const numStr = numValue.toLocaleString(undefined, {
  ...
  if (decimalArr.length > accuracy) decimalArr.length = accuracy

  return [integer, decimalArr.join('')].join('.')
}
